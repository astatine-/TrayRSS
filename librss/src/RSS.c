...
RSS_Feed* RSS_parse_RSS(const RSS_Node* root, RSS_error_handler handler)
{
	...
	while(node)
	{
		// some node->values contain just spaces (e.g. bbc rss feeds) and hence the earlier check of a valid value field succeeds even for
		// a new item. this forces the value to be having a non-spaces content for it to be considered a real value
		// assumes that the first test passess before the strlen is called and hence no additional check for node->value there
		if(node->value && RSS_strlen(RSS_strtrim(node->value)) > 0)
		{
